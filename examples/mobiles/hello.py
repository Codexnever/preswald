...
selected_battery_capacity = slider(
    "Select Battery Capacity", min_val=2000, max_val=10000, default=3000
)
...
df1["Battery Capacity"] = (
    df1["Battery Capacity"].str.replace(",", "").str.replace("mAh", "").astype(float)
)
...
if filtered_df.empty:
    ...
    avg_price_brand_battery = (
        filtered_df.groupby("Company Name")["Launched Price (USA)"].mean().reset_index()
    )
...
selected_column = selectbox(
    label="Choose a Launch Price Region", options=list_launched_price_col
)  # Default to USA if input is invalid

df[selected_column] = df[selected_column].apply(
    clean_price
)  # cleaning the price data for selected column

# Filter data based on selected region
region_price_data = (
    df.groupby("Company Name")[selected_column].mean().sort_values(ascending=False)
)

# Create interactive bar chart
fig_launch_price_region = go.Figure()

fig_launch_price_region.add_trace(
    go.Bar(
        x=region_price_data.values,
        y=region_price_data.index,
        orientation="h",
        marker={"color": region_price_data.values, "colorscale": "viridis"},
...
search_query = text_input(
    label="Enter your Phone Name", placeholder="iPhone"
)  # Example: User searches for "iPhone"

# Filter data where the Model Name or Company Name contains the search term
filtered_df = df[
    df["Model Name"].str.contains(search_query, case=False, na=False)
