...
try:
        ...
        for path in possible_paths:
            if os.path.exists(path):
                try:
                    ...
                    text(f"Error loading from {path}: {e!s}")

    if df is None:
        text(
            "⚠️ Could not load the Iris dataset. Creating sample data for demonstration."
        )
        df = pd.DataFrame(
            {
    ...
    if "sepal_length" in df.columns:
        ...
        text(
            f"Showing {len(filtered_df)} flowers with sepal length ≥ {sepal_length_threshold:.1f} cm"
        )
        table(filtered_df, title="Filtered Results")

except Exception as e:
    text(f"⚠️ An error occurred: {e!s}")
